...
export const articlePayloadSchema = k
  .object({
...
// ------------------ Validators ------------------
export async function validatePostsQuery(
  data: unknown,
): Promise<ValidationResult<PostsQuery>> {
  const result: ReturnType<typeof postsQuerySchema.safeParse> =
    postsQuerySchema.safeParse(data);
  if (!result.success) {
...
export async function validateCreatePostBody(
  data: unknown,
): Promise<ValidationResult<UnifiedCreatePost>> {
  // Validate against the unified schema supporting multiple post types
  const result: ReturnType<typeof unifiedCreatePostSchema.safeParse> =
    unifiedCreatePostSchema.safeParse(data);
  if (!result.success) {
    const errors = result.issues.map((i) => ({
      field: i.path.join("."),
      message: i.message,
...
export async function validateUpdatePostBody(
  data: unknown,
): Promise<ValidationResult<UnifiedUpdatePost>> {
  const result: ReturnType<typeof unifiedUpdatePostSchema.safeParse> =
    unifiedUpdatePostSchema.safeParse(data);
  if (!result.success) {
    const errors = result.issues.map((i) => ({
      field: i.path.join("."),
      message: i.message,
